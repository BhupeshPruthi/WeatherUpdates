...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if cities.count > 0 {
            ...
            print("*********** DB Does not contains data ***********")
            let locationSearchViewController = storyboard.instantiateViewController(withIdentifier: "LocationSearchVC") as! LocationSearchViewController
            let initalViewController = UINavigationController(rootViewController: locationSearchViewController)
            self.window?.rootViewController = initalViewController
        }
